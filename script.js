...
function createFirework(x, y, isFinal = false) {
    const firework = document.createElement('div');
    firework.className = 'fixed pointer-events-none z-50';
    firework.style.left = x + 'px';
    firework.style.top = y + 'px';
    
    const particles = isFinal ? 12 : 8;
    const size = isFinal ? 8 : 6;
    const duration = isFinal ? 1200 : 800;
...
// Функция для запуска салютов при клике на логотип
function setupFireworks() {
    const logo = document.getElementById('companyLogo');
    if (!logo) return;
    
    logo.addEventListener('click', function(e) {
        // Запускаем 4 случайных салюта
        for (let i = 0; i < 4; i++) {
            setTimeout(() => {
                const x = Math.random() * window.innerWidth;
                const y = Math.random() * window.innerHeight;
                createFirework(x, y, false);
            }, i * 200); // Задержка между салютами
        }
        
        // Пятый салют в центре экрана с задержкой
        setTimeout(() => {
            const centerX = window.innerWidth / 2;
            const centerY = window.innerHeight / 2;
            createFirework(centerX, centerY, true);
        }, 1000);
    ...
    logo.title = 'Нажмите для праздничного салюта!';
}
